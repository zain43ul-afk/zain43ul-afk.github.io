(() => {
  ...
  if (finePointer && !reducedMotion) {
    ...
    const renderHeroMotion = () => {
      heroMotionFrame = 0;
      hero?.style.setProperty("--hero-parallax-x", `${heroMotionX.toFixed(2)}px`);
      hero?.style.setProperty("--hero-parallax-y", `${heroMotionY.toFixed(2)}px`);
    };

    hero?.addEventListener("pointermove", (event) => {
      const bounds = hero.getBoundingClientRect();
      heroMotionX = ((event.clientX - bounds.left) / bounds.width - .5) * 18;
      heroMotionY = ((event.clientY - bounds.top) / bounds.height - .5) * 12;
      if (!heroMotionFrame) heroMotionFrame = window.requestAnimationFrame(renderHeroMotion);
    });
